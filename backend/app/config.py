...
class Settings(BaseSettings):
    ...
    db_pool_size: int = Field(5, validation_alias="DB_POOL_SIZE", ge=1, le=100)
    db_max_overflow: int = Field(10, validation_alias="DB_MAX_OVERFLOW", ge=0, le=200)
    db_pool_timeout_seconds: int = Field(
        30, validation_alias="DB_POOL_TIMEOUT_SECONDS", ge=1, le=300
    )
    db_pool_recycle_seconds: int = Field(
        1800, validation_alias="DB_POOL_RECYCLE_SECONDS", ge=0, le=86400
    )
    db_statement_timeout_ms: int = Field(
        30000, validation_alias="DB_STATEMENT_TIMEOUT_MS", ge=1, le=300000
    )
    db_idle_transaction_timeout_ms: int = Field(
        60000, validation_alias="DB_IDLE_TRANSACTION_TIMEOUT_MS", ge=1, le=600000
    )
    ingestion_max_file_bytes: int = Field(
        10 * 1024 * 1024, validation_alias="INGESTION_MAX_FILE_BYTES", ge=1, le=100 * 1024 * 1024
    )
    ingestion_max_records: int = Field(
        1000, validation_alias="INGESTION_MAX_RECORDS", ge=1, le=100000
    )
