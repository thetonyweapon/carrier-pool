...
FastAPI foundation, a React/Vite operations console, and a durable polling
worker that discovers read-only source files into a leased ingestion queue.
...
flowchart TB
    ...
    files -->|Read-only chronological input| worker
    worker -->|Normalized records\ntransactions and audit history| db
    ...
    class api,worker,db,files,rec,rate,ui delivered;
...
| Ingestion worker | Discover source files, lease ordered jobs, retry safely, and dead-letter failures | Delivered |
...
- Docker Compose runs PostgreSQL, the backend, the durable ingestion worker, and
  the React/Vite console.
...
- The worker polls every 60 seconds by default (configurable in production),
  renews leases while processing, and records retryable failures or dead letters
  in PostgreSQL.
