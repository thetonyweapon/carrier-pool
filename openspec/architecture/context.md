...
**Status: Delivered baseline with planned product capabilities**

Carrier Pool ingests broker-owned TMS exports into a canonical, tenant-scoped
data model. The current implementation provides ingestion and health behavior;
recommendations, rate estimates, UI workflows, and shared-pool behavior are
planned.
...
flowchart LR
    ...
    class platform,files,postgres delivered;
    class recommendations,estimates,pool planned;
...
- Future recommendations and estimates must use broker-owned history by default.
